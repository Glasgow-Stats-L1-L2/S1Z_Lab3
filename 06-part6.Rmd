# Optional tasks (without full code)
...
Combine your graph from Exercise 9 with the fitted regression line from the model superimposed on top. Rather than using `geom_smooth()` to do this, try using `geom_abline` with the appropriate values for the slope and intercept from the model.
...
ggplot(data = mlb11, mapping = aes(x = new_slug, y = runs)) +
    geom_point() +
    labs(title = "Relationship between Runs and Slugging Prop.", x = "Slugging Prop.", y = "Runs")+ 
    geom_abline(intercept = -375.80, slope = 2681.33)

ggplot(data = mlb11, mapping = aes(x = new_slug, y = runs)) +
    geom_point() +
    labs(title = "Relationship between Runs and Slugging Prop.", x = "Slugging Prop.", y = "Runs")+ 
    geom_abline(intercept = coef(m3)[["(Intercept)"]], 
                slope = coef(m3)[["new_slug"]])
```
